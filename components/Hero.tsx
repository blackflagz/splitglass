...
import { ArrowDown, Users, CheckCircle } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          >
            ...
            <h1 className={`font-anton tracking-wide mb-8 text-white relative uppercase ${language === 'ar'
              ? "text-[2rem] md:text-[3.5rem] lg:text-[3.5rem] font-bold leading-[1.3]"
              : language === 'fr'
                ? "text-[2.5rem] md:text-[4rem] lg:text-[4.5rem] leading-[1]"
            ...
            <p className={`text-white/60 mb-6 max-w-xl mx-auto lg:mx-0 leading-relaxed font-light ${language === 'ar' ? "text-lg md:text-xl" : "text-xl md:text-2xl"
              }`}>
              {t('hero.desc')}
            </p>

            {/* Supporting bullets */}
            <ul className="flex flex-col gap-2 mb-10 max-w-xl mx-auto lg:mx-0">
              {['hero.bullet1', 'hero.bullet2', 'hero.bullet3'].map((key) => (
                <li key={key} className="flex items-start gap-2 text-white/50 text-sm">
                  <CheckCircle className="w-4 h-4 text-[#2979FF] mt-0.5 flex-shrink-0" />
                  <span>{t(key)}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto lg:mx-0 mb-10">
              <a href="#contact" className="w-full sm:w-auto">
                <PremiumButton
                  variant="primary"
                  size="md"
                  className="rounded-xl whitespace-nowrap shadow-[0_0_20px_rgba(41,121,255,0.3)] h-[46px] w-full"
                >
                  {t('hero.cta')}
                </PremiumButton>
              </a>
              <a href="#waitlist" className="w-full sm:w-auto">
                <PremiumButton
                  variant="secondary"
                  size="md"
                  className="rounded-xl whitespace-nowrap h-[46px] w-full"
                >
                  {t('hero.cta2')}
                </PremiumButton>
              </a>
